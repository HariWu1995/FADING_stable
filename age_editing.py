...
parser.add_argument('--save_dir', default='./outputs', help='Path to save outputs')
...
seeder = torch.Generator(device=device)

ldm_stable = StableDiffusionPipeline.from_pretrained(model_path,
                                                    scheduler=scheduler,
...
(image_gt, image_enc), x_t, \
    uncond_embeddings = null_inversion.invert(image_path, inversion_prompt,
...
for age_new in target_ages:
    ...
    blend_word = (((str(age_init), person_placeholder,), 
                   (str(age_new), new_person_placeholder,)))
    ...
    controller = make_controller(prompts, is_replace_controller, 
                                cross_replace_steps, self_replace_steps,
                                tokenizer, blend_word, eq_params)

    images, _ = p2p_text2image(ldm_stable, prompts, controller, generator=seeder.manual_seed(0),
                               latent=x_t, uncond_embeddings=uncond_embeddings)

    new_img = images[-1]
    new_img_pil = Image.fromarray(new_img)
    new_img_pil.save(os.path.join(save_dir,f'{input_img_name}_{age_new}.png'))
